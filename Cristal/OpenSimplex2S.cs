/* Public domain (with modifications) from https://github.com/KdotJPG/OpenSimplex2 */
...
namespace Cristal {
    public static class OpenSimplex2S {
        ...
        /// Probably slightly less optimal for heightmaps or continent maps, unless your map is centered around an equator.
        /// It's a slight difference, but the option is here to make it easy.
        /// </summary>
        ...
        private static float Noise2_UnskewedBase(long seed,double xs,double ys) {
            ...
            long xsb_prime = xsb * PRIME_X;
            long ysb_prime = ysb * PRIME_Y;
            ...
            float value = (a0 * a0) * (a0 * a0) * Grad(seed,xsb_prime,ysb_prime,dx0,dy0);

            // Second vertex.
            float a1 = (float)(2 * (1 + 2 * UNSKEW_2D) * (1 / UNSKEW_2D + 2)) * t + ((float)(-2 * (1 + 2 * UNSKEW_2D) * (1 + 2 * UNSKEW_2D)) + a0);
            float dx1 = dx0 - (float)(1 + 2 * UNSKEW_2D);
            float dy1 = dy0 - (float)(1 + 2 * UNSKEW_2D);
            value += (a1 * a1) * (a1 * a1) * Grad(seed,xsb_prime + PRIME_X,ysb_prime + PRIME_Y,dx1,dy1);
            ...
            if(t < UNSKEW_2D) {
                if(xi + xmyi > 1) {
                    ...
                    if(a2 > 0) {
                        value += (a2 * a2) * (a2 * a2) * Grad(seed,xsb_prime + (PRIME_X << 1),ysb_prime + PRIME_Y,dx2,dy2);
                    }
                } else {
                    ...
                    float a2 = RSQUARED_2D - dx2 * dx2 - dy2 * dy2;
                    if(a2 > 0) {
                        value += (a2 * a2) * (a2 * a2) * Grad(seed,xsb_prime,ysb_prime + PRIME_Y,dx2,dy2);
                    }
                }
                ...
                if(yi - xmyi > 1) {
                    ...
                    if(a3 > 0) {
                        value += (a3 * a3) * (a3 * a3) * Grad(seed,xsb_prime + PRIME_X,ysb_prime + (PRIME_Y << 1),dx3,dy3);
                    }
                } else {
                    ...
                    float a3 = RSQUARED_2D - dx3 * dx3 - dy3 * dy3;
                    if(a3 > 0) {
                        value += (a3 * a3) * (a3 * a3) * Grad(seed,xsb_prime + PRIME_X,ysb_prime,dx3,dy3);
                    }
                }
            } else {
                if(xi + xmyi < 0) {
                    ...
                    float a2 = RSQUARED_2D - dx2 * dx2 - dy2 * dy2;
                    if(a2 > 0) {
                        value += (a2 * a2) * (a2 * a2) * Grad(seed,xsb_prime - PRIME_X,ysb_prime,dx2,dy2);
                    }
                } else {
        ...
        /// It was actually faster to narrow down the points in the loop itself, than to build up the index with enough info to isolate 8 points.
        /// </summary>
        private static float Noise3_UnrotatedBase(long seed,double xr,double yr,double zr) {
            ...
            long xrb_prime = xrb * PRIME_X;
            long yrb_prime = yrb * PRIME_Y;
            long zrb_prime = zrb * PRIME_Z;
            ...
            float value = (a0 * a0) * (a0 * a0) * Grad(seed,
                xrb_prime + (xNMask & PRIME_X),yrb_prime + (yNMask & PRIME_Y),zrb_prime + (zNMask & PRIME_Z),x0,y0,z0);
            ...
            value += (a1 * a1) * (a1 * a1) * Grad(seed2,
                xrb_prime + PRIME_X,yrb_prime + PRIME_Y,zrb_prime + PRIME_Z,x1,y1,z1);
            ...
            if(a2 > 0) {
                ...
                value += (a2 * a2) * (a2 * a2) * Grad(seed,
                    xrb_prime + (~xNMask & PRIME_X),yrb_prime + (yNMask & PRIME_Y),zrb_prime + (zNMask & PRIME_Z),x2,y2,z2);
            } else {
                ...
                if(a3 > 0) {
                    ...
                    value += (a3 * a3) * (a3 * a3) * Grad(seed,
                        xrb_prime + (xNMask & PRIME_X),yrb_prime + (~yNMask & PRIME_Y),zrb_prime + (~zNMask & PRIME_Z),x3,y3,z3);
                }
                ...
                if(a4 > 0) {
                    ...
                    value += (a4 * a4) * (a4 * a4) * Grad(seed2,
                        xrb_prime + (xNMask & unchecked(PRIME_X * 2)),yrb_prime + PRIME_Y,zrb_prime + PRIME_Z,x4,y4,z4);
                    skip5 = true;
                }
            }
            ...
            if(a6 > 0) {
                ...
                value += (a6 * a6) * (a6 * a6) * Grad(seed,
                    xrb_prime + (xNMask & PRIME_X),yrb_prime + (~yNMask & PRIME_Y),zrb_prime + (zNMask & PRIME_Z),x6,y6,z6);
            } else {
                ...
                if(a7 > 0) {
                    ...
                    value += (a7 * a7) * (a7 * a7) * Grad(seed,
                        xrb_prime + (~xNMask & PRIME_X),yrb_prime + (yNMask & PRIME_Y),zrb_prime + (~zNMask & PRIME_Z),x7,y7,z7);
                }
                ...
                if(a8 > 0) {
                    ...
                    value += (a8 * a8) * (a8 * a8) * Grad(seed2,
                        xrb_prime + PRIME_X,yrb_prime + (yNMask & (PRIME_Y << 1)),zrb_prime + PRIME_Z,x8,y8,z8);
                    skip9 = true;
                }
            }
            ...
            if(aA > 0) {
                ...
                value += (aA * aA) * (aA * aA) * Grad(seed,
                    xrb_prime + (xNMask & PRIME_X),yrb_prime + (yNMask & PRIME_Y),zrb_prime + (~zNMask & PRIME_Z),xA,yA,zA);
            } else {
                ...
                if(aB > 0) {
                    ...
                    value += (aB * aB) * (aB * aB) * Grad(seed,
                        xrb_prime + (~xNMask & PRIME_X),yrb_prime + (~yNMask & PRIME_Y),zrb_prime + (zNMask & PRIME_Z),xB,yB,zB);
                }
                ...
                if(aC > 0) {
                    ...
                    value += (aC * aC) * (aC * aC) * Grad(seed2,
                        xrb_prime + PRIME_X,yrb_prime + PRIME_Y,zrb_prime + (zNMask & (PRIME_Z << 1)),xC,yC,zC);
                    skipD = true;
                }
            }
            ...
            if(!skip5) {
                ...
                if(a5 > 0) {
                    ...
                    value += (a5 * a5) * (a5 * a5) * Grad(seed2,
                        xrb_prime + PRIME_X,yrb_prime + (yNMask & (PRIME_Y << 1)),zrb_prime + (zNMask & (PRIME_Z << 1)),x5,y5,z5);
                }
            }
            ...
            if(!skip9) {
                ...
                if(a9 > 0) {
                    ...
                    value += (a9 * a9) * (a9 * a9) * Grad(seed2,
                        xrb_prime + (xNMask & unchecked(PRIME_X * 2)),yrb_prime + PRIME_Y,zrb_prime + (zNMask & (PRIME_Z << 1)),x9,y9,z9);
                }
            }
            ...
            if(!skipD) {
                ...
                if(aD > 0) {
                    ...
                    value += (aD * aD) * (aD * aD) * Grad(seed2,
                        xrb_prime + (xNMask & (PRIME_X << 1)),yrb_prime + (yNMask & (PRIME_Y << 1)),zrb_prime + PRIME_Z,xD,yD,zD);
                }
            }
        ...
        /// <summary>
        /// 4D SuperSimplex noise, with XYZ oriented like Noise3_ImproveXZ and W for an extra degree of freedom. W repeats eventually.
        ...
        /// <summary>
        /// 4D SuperSimplex noise, with XYZ oriented like Noise3_Fallback and W for an extra degree of freedom. W repeats eventually.
        /// Recommended for time-varied animations which texture a 3D object (W=time) where there isn't a clear distinction between horizontal and vertical.
        /// </summary>
        ...
        /// This isn't as elegant or SIMD/GPU/etc. portable as other approaches, but it competes performance-wise with optimized 2014 OpenSimplex.
        /// </summary>
        private static float Noise4_UnskewedBase(long seed,double xs,double ys,double zs,double ws) {
            ...
            for(int i = secondaryIndexStart;i < secondaryIndexStop;i++) {
                ...
                if(a < RSQUARED_4D) {
                    a -= RSQUARED_4D;
                    a *= a;
                    value += a * a * Grad(seed,vx + c.vx,vy + c.vy,vz + c.vz,vw + c.vw,dx,dy,dz,dw);
                }
            }
            return value;
        }
        ...
        /// <summary>
        /// Two dimension gradient.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static float Grad(long seed,long vx,long vy,float dx,float dy) {
            long hash = seed ^ vx ^ vy;
        ...
        /// <summary>
        /// Three dimension gradient.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static float Grad(long seed,long vx,long vy,long vz,float dx,float dy,float dz) {
            long hash = (seed ^ vx) ^ (vy ^ vz);
            hash *= HASH_MULTIPLIER;
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static float Grad(long seed,long vx,long vy,long vz,long vw,float dx,float dy,float dz,float dw) {
            long hash = seed ^ (vx ^ vy) ^ (vz ^ vw);
            hash *= HASH_MULTIPLIER;
        ...
        private static readonly float[] GRADIENTS_2D, GRADIENTS_3D, GRADIENTS_4D;
        ...
        private readonly struct LatticeVertex4D {

            public readonly float dx, dy, dz, dw;

            public readonly long vx, vy, vz, vw;

            public LatticeVertex4D(int x,int y,int z,int w) {
                vx = x * PRIME_X;
                vy = y * PRIME_Y;
                vz = z * PRIME_Z;
                vw = w * PRIME_W;

                float ssv = (x + y + z + w) * UNSKEW_4D;
                dx = -x - ssv;
                dy = -y - ssv;
                dz = -z - ssv;
                dw = -w - ssv;
